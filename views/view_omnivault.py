...
def render_page():
    ...
    with st.sidebar:
        st.subheader("🔍 當前分類搜尋")
        omni_local_search = st.text_input("輸入關鍵字", placeholder="過濾當前畫面...", label_visibility="collapsed", key="omni_local")
        st.markdown("---")
        
        # 導航按鈕
        if st.button("🗂️ 返回總覽首頁", use_container_width=True):
            st.session_state.omni_active_category = None
        if st.button("🔍 進入全域搜尋", use_container_width=True):
            st.session_state.omni_active_category = "🔍 搜尋中心"
    ...
    # 🌟 獨立的搜尋中心分頁
    # ==========================================
    if st.session_state.omni_active_category == "🔍 搜尋中心":
        st.subheader("🔍 全域搜尋中心")
        st.caption("在此輸入關鍵字，系統將為您檢索所有的自訂分類。")
        global_q = st.text_input("跨分類搜尋：", placeholder="搜尋項目名稱、筆記...", key="omni_g")
        st.markdown("---")
        
        if not global_q:
            st.info("👈 請在上方輸入關鍵字開始檢索。")
        else:
            df_omni_g = core_utils.fetch_omni_items(search_query=global_q)
            if not df_omni_g.empty:
                st.markdown(f"#### 🗃️ 收藏項目 ({len(df_omni_g)} 筆)")
                for _, row in df_omni_g.iterrows():
                    st.markdown(f"- **[{row.get('title', '未命名')}]({row.get('url', '#') if pd.notna(row.get('url')) else '#'})** ｜ 📍 位於：`{row.get('category', '未知')}`")
            else:
                st.warning(f"找不到包含「{global_q}」的項目。")

    # ==========================================
    # 視圖 A：總覽首頁 (Pinterest 風格)
    # ==========================================
    elif st.session_state.omni_active_category is None:
        df_all_raw = core_utils.fetch_omni_items()
        existing_cats = core_utils.fetch_omni_categories()
        
        col_desc, col_add = st.columns([3, 1])
        with col_desc: st.caption("點擊下方分類資料夾，進入個人陳列室。")
        with col_add:
        ...
        if not existing_cats:
                    ...
                    with st.container(border=True):
                        if cover_img:
                            ...
                            <div style="position: relative; width: 100%; aspect-ratio: 16/10; overflow: hidden; border-radius: 6px; margin-bottom: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.15);">
                                ...
                                <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.45); display: flex; align-items: center; justify-content: center; font-size: 2rem;">{icon}</div>
                            </div>
                            """
            ...
            if is_edit_mode:
                ...
                # 🌟 首頁編輯模式也支援局域搜尋
                df_all_raw = core_utils.fetch_omni_items(search_query=omni_local_search)
    ...
    # ==========================================
        ...
        # 🌟 傳入局域搜尋
        df_omni = core_utils.fetch_omni_items(category=active_cat, search_query=omni_local_search)
                ...
                for idx, row in page_data.reset_index(drop=True).iterrows():
                    with cols[idx % 3]:
                        with st.container(border=True):
                            ...
                            if pd.notna(row['url']) and str(row['url']).strip(): st.markdown(f"#### [{row['title']}]({row['url']})")
