                    ...
                    with st.spinner("正在呼叫書籍 API..."):
                        ...
                        if book_data:
                            ...
                            try:
                                ...
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
                                ON CONFLICT(identifier) DO UPDATE SET is_bookmarked=1, title=excluded.title, image=excluded.image;
                                """
                                ...
                                st.cache_data.clear(); st.success(f"✅ 已成功將《{book_data['title']}》加入待讀書架！")
                            except Exception as e: st.error(f"寫入資料庫失敗: {e}")
                        else: st.error("❌ 找不到該 ISBN 的書籍資料。")
