...
SOURCE_URLS = {
    ...
    "Duke Arts & Humanities": "https://today.duke.edu/topics/arts-&-humanities",
}
        ...
        if df.empty:
                ...
                with st.container():
                    ...
                    with col_btn2:
                        with st.popover("🗑️ 刪除"):
                            st.markdown("⚠️ **確認刪除嗎？**")
                            st.button("✅ 確定", key=f"del_{row['Link']}", on_click=delete_article_db, args=(row['Link'],), type="primary", use_container_width=True)
            ...
            if total_pages > 1:
                ...
                with col_page:
                    ...
                    st.caption(f"目前顯示第 {st.session_state.mono_page} 頁，共 {total_pages} 頁")
