...
def update_book_status(item_ids, new_status):
    ...
    try:
        placeholders = ','.join(['?'] * len(item_ids))
        db.execute(f"UPDATE academic_pubs SET book_status = ? WHERE id IN ({placeholders})", [new_status] + item_ids)
        st.cache_data.clear()
        st.toast("📚 閱讀狀態已更新！")
    except Exception as e:
        st.error(f"狀態更新失敗: {e}")
    ...
    if tmdb_key:
        try:
            ...
            if data:
                ...
                return {
                    "media_type": media_type_label, "title": title, "creator": creator_name,
                    "cover_image": get_secure_image_base64(img_url, "tmdb") if img_url else None,
                    "source_url": target_url, "summary": summary if summary else '無簡介'
                }
        except Exception as e: print(f"TMDB 查詢失敗: {e}")
    ...
    try:
        ...
        if res.get("meta", {}).get("count", 0) > 0:
            ...
            return {
                ...
                "abstract": "（由 OpenAlex 標題盲搜匯入）", "link": doi_link or item.get("id", ""), "image": "", "book_status": 0
            }
...
def add_url_backup(url_book_data):
    try:
        sql = "INSERT INTO academic_pubs (type, title, author, publisher_journal, issue_volume, identifier, publish_date, abstract, link, image, book_status, is_manual, category) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 1, '未分類') ON CONFLICT(identifier) DO UPDATE SET title=excluded.title;"
        ...
        st.cache_data.clear()
        return True, f"📋 備存成功！已將《{url_book_data['title']}》強行歸檔至清單！"
    except Exception as e: return False, f"寫入失敗: {e}"
